...
async function freshDiscordAuthModule(sdk: ReturnType<typeof fakeSdk>) {
  ...
  mock.module('@/platform/api/apiBase', () => ({
  ...
  // handshake right away (see platform/discord/auth.ts) — that's the behavior under
...
describe('runAuthFlow', () => {
  ...
  it('clears the cache on failure so a later call retries', async () => {
    ...
    // The module self-starts on import (see platform/discord/auth.ts), so this may be
