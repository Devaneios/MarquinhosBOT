import { type PlayQueueMetadata } from '@marquinhos/commands/music/utils';
...
export async function initializePlayer(client: SapphireClient) {
  ...
  const handlePlayStart = async (
    queue: GuildQueue<PlayQueueMetadata>,
    track: Track,
  ) => {
    const { interactionChannel, addedBy } = queue.metadata;
  ...
  player.events.on(
    'playerStart',
    (queue: GuildQueue<PlayQueueMetadata>, track: Track) =>
  ...
  player.events.on(
    'playerError',
    (queue: GuildQueue<PlayQueueMetadata>, error: Error) => {
      logger.error(`Error in player: ${error.message}`);
      const { interactionChannel } = queue.metadata;
  ...
  if (env.DEEZER_ARL_COOKIE) {
    ...
    logger.warn(
      'Deezer extractor não registrado: DEEZER_ARL_COOKIE não configurado.',
    );
  }
}
