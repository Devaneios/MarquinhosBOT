...
export const envSchema = z
  .object({
    ...
    MARQUINHOS_ERROR_DM_USER_ID: z.string().min(1).optional(),
  ...
  .superRefine((value, context) => {
    if (value.NODE_ENV === 'development' && !value.DEV_TEST_CHANNEL_ID) {
      context.addIssue({
        code: 'custom',
        path: ['DEV_TEST_CHANNEL_ID'],
        message: 'DEV_TEST_CHANNEL_ID is required in development',
      });
    }
  });
