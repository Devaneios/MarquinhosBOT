...
export const QuestionScreen: React.FC<QuestionScreenProps> = ({
  ...
  const { t } = useTranslation(['trivia-quiz', 'games']);
    ...
    <div className="flex flex-1 flex-col overflow-hidden bg-[var(--color-marquinhos-bg)]">
      ...
      <main className="flex min-h-0 flex-1 flex-col gap-6 overflow-y-auto p-6">
        <div className="notch-8 w-full border border-marquinhos-border bg-marquinhos-panel px-6 py-5 shadow-[0_20px_40px_rgba(0,0,0,0.24)]">
          <h2 className="mb-4 text-lg font-semibold text-marquinhos-text">
            {state.currentQuestion.text}
          </h2>

          <div className="mb-4 space-y-2">
            <div className="flex justify-between text-xs text-marquinhos-text-dim">
              <span>{t('trivia-quiz:questionTimeLabel')}</span>
        ...
        {state.playerScores.length > 0 && (
          <div className="notch-8 border border-marquinhos-border bg-marquinhos-panel px-6 py-5 shadow-[0_20px_40px_rgba(0,0,0,0.24)]">
            <h3 className="mb-3 font-semibold text-marquinhos-text">
              {t('trivia-quiz:questionScoresTitle')}
            </h3>
            <div className="space-y-2 text-sm">
              {state.playerScores.map((ps) => (
                <div
                  key={ps.userId}
                  className="flex justify-between text-marquinhos-text-dim"
                >
                  <span>{ps.userId}</span>
