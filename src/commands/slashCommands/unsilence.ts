...
export const unsilence: SlashCommand = {
  ...
  execute: async (interaction) => {
    ...
    const unsilenced = interaction.options.get('liberado').member as GuildMember;
    ...
    const result = await findAndUnsilenceMember(unsilenced.id, unsilenced.user.username);
    ...
    interaction.reply( {content: `${unsilenced.nickname} já tava podendo falar..`});
