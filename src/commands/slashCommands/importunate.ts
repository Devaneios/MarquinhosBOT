...
export const importunate: SlashCommand = {
  command: new SlashCommandBuilder()
    .setName('importunar')
    .setDescription('Eu vou lá no canal de voz atazanar a vida de quem tu quiser.')
  ...
  execute: async (interaction) => {
    ...
    if (member.voice.channel) {
        ...
        interaction.reply({ content: 'Acho que essa pessoa aí não tá num canal de voz..' })
    }
  },
  cooldown: 10,
};

function makeNoise(voiceChannel: VoiceChannel, interaction: CommandInteraction) {
