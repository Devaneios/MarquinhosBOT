import { Collection, GuildBasedChannel, GuildMember, Message, VoiceBasedChannel, VoiceChannel } from 'discord.js';
...
export const chaos: Command = {
  ...
  execute: (message: Message, args: string[]) => {
    ...
    if (levelOfChaos > 25) {
      message.channel.send('Não posso fazer isso, vocês não aguentariam tamanho caos!');
      return;
    }
...
async function chaos2(message: Message, limit: number) {
  ...
  if(voiceChannel!=undefined && voiceChannels!=undefined && activeUsers!=undefined) {
...
async function chaos3(counter: number, voiceChannel: VoiceBasedChannel, voiceChannels: Collection<string, GuildBasedChannel>, activeUsers: Collection<string, GuildMember>, limit: number) {
