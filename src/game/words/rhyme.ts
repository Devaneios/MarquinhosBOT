...
export class RhymeGame extends BaseGame {
  private readonly rhymeWords = [
    {
      word: 'AMOR',
      rhymes: ['DOR', 'FLOR', 'COR', 'SENHOR', 'VALOR', 'CALOR', 'TAMBOR', 'SABOR', 'TERROR', 'RUMOR']
    },
    {
      word: 'CASA',
      rhymes: ['MASSA', 'PASSA', 'PRASSA', 'CAÇA', 'PRAÇA', 'RAÇA', 'GRAÇA', 'ESPAÇA', 'ABRAÇA']
    },
    {
      word: 'CORAÇÃO',
      rhymes: ['AÇÃO', 'PAIXÃO', 'EMOÇÃO', 'CANÇÃO', 'LIÇÃO', 'RAZÃO', 'PRESSÃO', 'VERSÃO', 'MISSÃO']
    },
    {
      word: 'VIDA',
      rhymes: ['BEBIDA', 'COMIDA', 'PERDIDA', 'QUERIDA', 'FERIDA', 'PARTIDA', 'SAÍDA', 'CORRIDA']
    },
    {
      word: 'FELIZ',
      rhymes: ['NARIZ', 'RAIZ', 'CICATRIZ', 'DESLIZ', 'CHAFARIZ', 'MATRIZ', 'MOTRIZ', 'ATRIZ']
    },
    {
      word: 'CANTAR',
      rhymes: ['AMAR', 'SONHAR', 'VOAR', 'DANÇAR', 'BRINCAR', 'ESTUDAR', 'TRABALHAR', 'JOGAR']
    }
  ];
  ...
  private async submitRhyme(userId: string, rhyme: string): Promise<void> {
    ...
    const alreadyUsed = Object.values(data.playerRhymes)
      .flat()
      .includes(rhyme);
  ...
  getGameEmbed(): EmbedBuilder {
    ...
    if (data.finished) {
      ...
      const unusedRhymes = data.validRhymes.filter(r => !usedRhymes.includes(r));
