...
export class SecretWordGame extends BaseGame {
  private readonly words: WordData[] = [
    // Easy words
    { word: 'CASA', category: 'Objetos', difficulty: 'easy', hint: 'Onde você mora' },
    { word: 'GATO', category: 'Animais', difficulty: 'easy', hint: 'Animal doméstico que faz miau' },
    { word: 'ÁGUA', category: 'Natureza', difficulty: 'easy', hint: 'Líquido essencial para a vida' },
    { word: 'SOL', category: 'Natureza', difficulty: 'easy', hint: 'Estrela do nosso sistema solar' },
    { word: 'AMOR', category: 'Sentimentos', difficulty: 'easy', hint: 'Sentimento profundo de carinho' },
    
    // Medium words
    { word: 'COMPUTADOR', category: 'Tecnologia', difficulty: 'medium', hint: 'Máquina para processar dados' },
    { word: 'CHOCOLATE', category: 'Comida', difficulty: 'medium', hint: 'Doce feito com cacau' },
    { word: 'ELEFANTE', category: 'Animais', difficulty: 'medium', hint: 'Maior mamífero terrestre' },
    { word: 'BIBLIOTECA', category: 'Lugares', difficulty: 'medium', hint: 'Local com muitos livros' },
    { word: 'GUITARRA', category: 'Instrumentos', difficulty: 'medium', hint: 'Instrumento de cordas do rock' },
    { word: 'PROGRAMAÇÃO', category: 'Tecnologia', difficulty: 'medium', hint: 'Arte de criar software' },
    
    // Hard words
    { word: 'EXTRAORDINÁRIO', category: 'Adjetivos', difficulty: 'hard', hint: 'Algo fora do comum' },
    { word: 'PARALELEPÍPEDO', category: 'Geometria', difficulty: 'hard', hint: 'Forma geométrica 3D' },
    { word: 'INCONSTITUCIONAL', category: 'Jurídico', difficulty: 'hard', hint: 'Contrário à constituição' },
    { word: 'OTORRINOLARINGOLOGISTA', category: 'Profissões', difficulty: 'hard', hint: 'Médico especialista em ouvido, nariz e garganta' },
    { word: 'PNEUMOULTRAMICROSCOPICOSSILICOVULCANOCONIÓTICO', category: 'Medicina', difficulty: 'hard', hint: 'Doença pulmonar causada por inalação de partículas' }
  ];
  ...
  getGameEmbed(): EmbedBuilder {
    ...
    if (data.wrongLetters.length > 0) {
      const wrongDisplay = data.wrongLetters.filter(l => !l.includes('(palavra)')).sort().join(', ');
      if (wrongDisplay) {
  ...
  getLetterButtons() {
    ...
    const availableLetters = alphabet.filter(letter => 
      !data.guessedLetters.includes(letter) && !data.wrongLetters.includes(letter)
    );
  ...
  async finish(): Promise<GameResult> {
    ...
    this.session.players.forEach((player, index) => {
      const baseRewards = this.calculateRewards(player, data.won ? 1 : this.session.players.length);
      
      // Bonus XP for winning
      if (data.won) {
        baseRewards.xp += this.getDifficultyBonus() / 5;
        baseRewards.xp += Math.max(0, (data.maxWrongGuesses - data.wrongLetters.length) * 2);
      }
