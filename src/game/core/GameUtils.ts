...
export class GameUtils {
  ...
  static createGameEmbed(title: string, description: string, color?: number): EmbedBuilder {
  ...
  static getGameStateEmoji(state: GameState): string {
    switch (state) {
      ...
      case GameState.ACTIVE: return '🎮';
      case GameState.PAUSED: return '⏸️';
      case GameState.FINISHED: return '🏁';
      case GameState.CANCELLED: return '❌';
      default: return '❓';
    }
  }
  ...
  static validateGameAction(session: GameSession, userId: string): { valid: boolean, reason?: string } {
  ...
  static createWinnerEmbed(winners: GamePlayer[], gameType: GameType): EmbedBuilder {
