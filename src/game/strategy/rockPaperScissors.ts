...
export class RockPaperScissorsGame extends BaseGame {
  ...
  private async submitChoice(userId: string, choice: string): Promise<void> {
    ...
    const allChosen = this.session.players.every(p => data.playerChoices[p.userId]);
  ...
  private determineRoundWinners(): string[] {
    ...
    const nonEmptyChoices = Object.entries(choiceGroups).filter(([_, players]) => players.length > 0);
    ...
    if (nonEmptyChoices.length === 2) {
      ...
      if (this.choices[choice1 as keyof typeof this.choices].beats === choice2) {
        return players1;
      } else {
        return players2;
      }
  ...
  getGameEmbed(): EmbedBuilder {
    ...
    if (data.finished) {
      ...
      if (data.waitingForChoices) {
        ...
        const chosen = this.session.players.filter(p => data.playerChoices[p.userId]);
        const waiting = this.session.players.filter(p => !data.playerChoices[p.userId]);
    ...
    if (data.roundResults.length > 0) {
      ...
      if (lastResult.winners.length > 0) {
        const winners = lastResult.winners.map(id => 
          this.session.players.find(p => p.userId === id)?.username
        ).join(', ');
    ...
    return GameUtils.createGameEmbed('✂️ Pedra, Papel, Tesoura', description, color);
  }
  ...
  getChoiceButtons() {
    ...
    return [
      GameUtils.createGameButtons({
        ...
        styles: [ButtonStyle.Secondary, ButtonStyle.Secondary, ButtonStyle.Secondary]
      })
    ];
  }
