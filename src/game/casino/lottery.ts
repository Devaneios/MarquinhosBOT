...
export class LotteryGame extends BaseGame {
  ...
  private quickPick(): void {
    ...
    while (data.playerNumbers.length < this.numbersToSelect) {
      const number = Math.floor(Math.random() * this.maxNumber) + this.minNumber;
  ...
  getGameEmbed(): EmbedBuilder {
    ...
    const color = data.drawn ? 
      (data.matches >= 4 ? 0x00ff00 : data.matches >= 2 ? 0xffaa00 : 0xff6b6b) : 
      0x3498db;

    return GameUtils.createGameEmbed('🎫 Loteria do Marquinhos', description, color);
  }
  ...
  getNumberButtons() {
    ...
    for (let start = 1; start <= this.maxNumber; start += buttonsPerRow * 2) {
      ...
      if (labels.length > 0) {
        buttons.push(GameUtils.createGameButtons({
          labels,
          customIds,
          styles,
          disabled: labels.map(label => {
            const num = parseInt(label);
            return data.playerNumbers.includes(num) ? false : data.playerNumbers.length >= this.numbersToSelect;
          })
        }));
      }
  ...
  getActionButtons() {
    ...
    return [
      GameUtils.createGameButtons({
        ...
        styles: [ButtonStyle.Secondary, ButtonStyle.Danger, ButtonStyle.Primary],
