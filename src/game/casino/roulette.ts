...
export class RouletteGame extends BaseGame {
  ...
  private async pullTrigger(): Promise<void> {
    const data = this.session.data as RouletteData;
    const currentPlayer = data.mode === 'multiplayer' 
    ...
    if (hasBullet) {
      ...
      if (data.mode === 'multiplayer') {
        ...
        } while (!data.players[data.currentPlayerIndex].alive && data.players.filter(p => p.alive).length > 1);
      }
  ...
  private updateScores(): void {
    ...
    if (data.mode === 'solo') {
      ...
      this.updatePlayerScore(this.session.players[0].userId, survivalBonus + completionBonus);
    } else {
      // Multiplayer scoring
      data.players.forEach(roulettePlayer => {
        const player = this.session.players.find(p => p.userId === roulettePlayer.userId);
        if (player) {
          const score = roulettePlayer.survived * 15 + (roulettePlayer.alive ? 50 : 0);
  ...
  getGameEmbed(): EmbedBuilder {
    ...
    const color = data.gameOver ? (data.result === 'survived' || data.players.filter(p => p.alive).length > 0 ? 0x00ff00 : 0xff0000) : 0xffaa00;
  ...
  async finish(): Promise<GameResult> {
    ...
    return {
      ...
      stats: {
        ...
        totalSurvived: data.mode === 'solo' ? data.survived : data.players.reduce((sum, p) => sum + p.survived, 0),
