...
export const analytics: SlashCommand = {
  command: new SlashCommandBuilder()
    ...
    .addSubcommand(subcommand =>
      subcommand
        .setName('user')
        .setDescription('Suas estatísticas pessoais')
    )
    .addSubcommand(subcommand =>
      subcommand
        .setName('server')
...
async function handleUserAnalytics(interaction: any, userId: string, guildId: string) {
...
async function handleServerReport(interaction: any, guildId: string) {
  ...
  const embed = interaction.client.baseEmbed()
    ...
    .addFields(
      ...
      { name: '🎮 Comando Mais Usado', value: '/play (234 vezes)', inline: true },
