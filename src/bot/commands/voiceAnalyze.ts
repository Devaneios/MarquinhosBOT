...
export const voiceAnalyze: SlashCommand = {
  command: new SlashCommandBuilder()
    ...
    .setDescription('Analisa sua performance vocal em tempo real durante karaokê')
  ...
  execute: async (interaction) => {
    ...
    try {
      ...
      const response = await marquinhosApi.post('/voice-ai/analyze', analysisData);
      ...
      const embed = interaction.client.baseEmbed()
        ...
        .addFields(
          { name: '🎯 Afinação', value: `${analysis.pitchAccuracy}%`, inline: true },
          { name: '⏱️ Timing', value: `${analysis.timingAccuracy}%`, inline: true },
          { name: '✨ Expressividade', value: `${analysis.expressiveness}%`, inline: true },
          { name: '🏆 Pontuação Total', value: `**${analysis.totalScore}/100**`, inline: false },
          { name: '💡 Feedback', value: analysis.feedback.join('\n'), inline: false }
        )
        .setColor(analysis.totalScore > 80 ? '#00FF00' : analysis.totalScore > 60 ? '#FFFF00' : '#FF0000');

      await interaction.editReply({ embeds: [embed] });
    } catch (error) {
      console.error('Error analyzing voice:', error);
      await interaction.editReply({ content: 'Erro ao analisar a voz. Tente novamente mais tarde.' });
    }
  },
  cooldown: 5,
};
