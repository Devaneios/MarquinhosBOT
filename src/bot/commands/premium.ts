...
export const premium: SlashCommand = {
  command: new SlashCommandBuilder()
    ...
    .addSubcommand(subcommand =>
      subcommand
        .setName('status')
        .setDescription('Status da sua assinatura')
    )
    .addSubcommand(subcommand =>
      subcommand
        .setName('donate')
  ...
  execute: async (interaction) => {
    ...
    switch (subcommand) {
      ...
      case 'subscribe':
        ...
        const subscribeEmbed = interaction.client.baseEmbed()
          .setTitle('💳 Assinatura Premium')
          .setDescription(`Você selecionou o plano **${planNames[plan as keyof typeof planNames]}**`)
          .addFields(
            { name: 'Próximo Passo', value: 'Acesse nosso site para finalizar a compra', inline: false },
            { name: 'Link de Pagamento', value: 'https://marquinhos.dev/premium', inline: false }
          );
      ...
      case 'status':
        const statusEmbed = interaction.client.baseEmbed()
      ...
      case 'donate':
        const donateEmbed = interaction.client.baseEmbed()
