    ...
    try {
      ...
      switch (subcommand) {
        case 'create':
          ...
          const democraticControl = interaction.options.getBoolean('democratic') ?? false;

          const createResponse = await marquinhosApi.post('/sync-party/create', {
          ...
          const createEmbed = interaction.client.baseEmbed()
            .setTitle('🎉 Festa Sincronizada Criada!')
            .setDescription(`**${name}**\n${description}`)
            .addFields(
              { name: '🆔 ID da Festa', value: `\`${party.id}\``, inline: true },
              { name: '🎵 Configurações', value: `Votação: ${allowVoting ? '✅' : '❌'}\nControle Democrático: ${democraticControl ? '✅' : '❌'}`, inline: true },
              { name: '📱 Como Participar', value: `Use \`/sync-party join party-id:${party.id}\``, inline: false }
            );
        ...
        case 'join':
          ...
          const joinResponse = await marquinhosApi.post(`/sync-party/${partyId}/join`, {
            userId: interaction.user.id,
            platform
          });

          if (!joinResponse.data) {
            await interaction.editReply({ content: 'Festa não encontrada ou inativa.' });
            return;
          }

          const joinedParty = joinResponse.data;

          const joinEmbed = interaction.client.baseEmbed()
            .setTitle('🎊 Entrou na Festa!')
            .setDescription(`Você entrou na festa **${joinedParty.name}**`)
            .addFields(
              { name: '👥 Participantes', value: `${joinedParty.participants.length}`, inline: true },
              { name: '📱 Sua Plataforma', value: platform, inline: true },
              { name: '🎵 Música Atual', value: joinedParty.currentTrack?.title || 'Nenhuma tocando', inline: false }
            );
        ...
        case 'add-track':
          ...
          const addResponse = await marquinhosApi.post(`/sync-party/${addPartyId}/track`, {
            userId: interaction.user.id,
            track: {
              title: title.trim(),
              artist: artist.trim(),
          ...
          if (!addResponse.data) {
            await interaction.editReply({ content: 'Festa não encontrada ou você não é participante.' });
            return;
          }

          const addEmbed = interaction.client.baseEmbed()
            .setTitle('🎵 Música Adicionada!')
            .setDescription(`**${title}** por **${artist}** foi adicionada à playlist`)
            .addFields(
              { name: '📋 Posição na Playlist', value: `#${addResponse.data.playlist.length}`, inline: true }
            );
        ...
        case 'vote':
          ...
          const trackNumber = interaction.options.getInteger('track-number', true);
          const voteType = interaction.options.getString('vote-type', true) as 'up' | 'down' | 'next';

          const voteResponse = await marquinhosApi.post(`/sync-party/${votePartyId}/vote`, {
            userId: interaction.user.id,
          ...
          if (!voteResponse.data) {
            await interaction.editReply({ content: 'Festa não encontrada ou votação desabilitada.' });
            return;
          }

          const voteEmojis = { up: '👍', down: '👎', next: '⏭️' };
          const voteNames = { up: 'curtiu', down: 'não curtiu', next: 'quer pular' };

          const voteEmbed = interaction.client.baseEmbed()
            .setTitle(`${voteEmojis[voteType]} Voto Registrado!`)
            .setDescription(`Você ${voteNames[voteType]} a música #${trackNumber}`);

          await interaction.editReply({ embeds: [voteEmbed] });
          break;

        case 'list':
          const listResponse = await marquinhosApi.get(`/sync-party/active/${interaction.guildId}`);
          const activeParties = listResponse.data;

          if (!activeParties || activeParties.length === 0) {
            await interaction.editReply({ content: 'Nenhuma festa ativa no momento. Crie uma com `/sync-party create`!' });
            return;
          }

          const partiesText = activeParties.map((p: any, index: number) => 
            `**${index + 1}.** ${p.name}\n` +
      ...
      await interaction.editReply({ content: 'Erro ao gerenciar festa sincronizada. Tente novamente mais tarde.' });
    }
  },
  cooldown: 5,
};
