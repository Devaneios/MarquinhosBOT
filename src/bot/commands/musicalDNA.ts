...
export const musicalDNA: SlashCommand = {
  ...
  execute: async (interaction) => {
    ...
    try {
      ...
      switch (subcommand) {
        case 'generate':
          const dnaResponse = await marquinhosApi.post(
            '/musical-dna/generate',
            {
          ...
          const topInstruments = Array.from(dna.instrumentPreferences.entries())
            ...
            .map(
              ([instrument, preference]) =>
                `${instrument}: ${Math.round(preference * 100)}%`,
            )
            .join('\n');
          ...
          const embed = interaction.client
            ...
            .addFields(
              {
                name: '🎵 BPM Preferido',
                value: `${dna.preferredBPM.min}-${dna.preferredBPM.max} (pico: ${dna.preferredBPM.peak})`,
                inline: true,
              },
              {
                name: '🎶 Harmonias',
                value: `Maior: ${Math.round(dna.harmonicPreferences.majorKeys * 100)}%\nMenor: ${Math.round(dna.harmonicPreferences.minorKeys * 100)}%`,
                inline: true,
              },
              {
                name: '🎸 Instrumentos Favoritos',
                value: topInstruments || 'Nenhum ainda',
                inline: false,
              },
            )
        ...
        case 'compatibility':
          ...
          const compatibilityResponse = await marquinhosApi.post(
            '/musical-dna/compatibility',
            {
          ...
          const compatEmbed = interaction.client
            ...
            .setDescription(
              `Compatibilidade entre ${interaction.user.username} e ${targetUser.username}`,
            )
            .addFields(
              {
                name: '💖 Pontuação',
              ...
              {
                name: '🎯 Nível',
                value: getCompatibilityLevel(compatibility),
                inline: true,
              },
            )
            .setColor(
        ...
        case 'evolution':
          const evolutionResponse = await marquinhosApi.get(
            `/musical-dna/evolution/${interaction.user.id}/${interaction.guildId}`,
          );

          if (!evolutionResponse.data) {
            await interaction.editReply({
              content:
                'Você ainda não tem dados de evolução suficientes. Continue ouvindo música!',
            });
            return;
          }
          ...
          const evolutionEmbed = interaction.client
            ...
            .setDescription(
              `Período: ${evolution.timespan.months} meses de dados`,
            )
            .addFields(
              {
                name: '🎵 BPM',
                value: `${evolution.changes.bpmEvolution.initial} → ${evolution.changes.bpmEvolution.current}`,
                inline: true,
              },
              {
                name: '📊 Tendência',
                value: evolution.trends.join('\n') || 'Estável',
                inline: false,
              },
            );
      ...
      await interaction.editReply({
        content: 'Erro ao processar DNA musical. Tente novamente mais tarde.',
      });
    }
  },
  cooldown: 10,
