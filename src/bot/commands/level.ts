...
export const level: SlashCommand = {
  ...
  execute: async (interaction) => {
    ...
    try {
      ...
      if (!userLevel?.data) {
        await interaction.editReply(`${targetUser.username} ainda não possui níveis registrados.`);
        return;
      }
      ...
      const embed = interaction.client.baseEmbed()
        ...
        .addFields(
          ...
          { name: '📊 Progresso', value: `${progressBar} ${progressPercentage}%`, inline: false }
        );

      await interaction.editReply({ embeds: [embed] });
    } catch (error) {
      console.error('Error fetching user level:', error);
      await interaction.editReply('Ocorreu um erro ao buscar as informações de nível.');
    }
  },
  cooldown: 10,
};
